...
class GaussianNoiseLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=None):
        if training:
            ...
            mask_1_F = tf.ones_like(inputs[:, :, :44])
            mask_0 = tf.zeros_like(inputs[:, :, 44:52])
            mask_1_L = tf.ones_like(inputs[:, :, 52:])
            mask = tf.concat([mask_1_F, mask_0, mask_1_L], axis=-1)
...
CHANNELS = 218 # number of keypoint value
NUM_CLASSES = 20
PAD = -100
...
def load_model(path='1DCNN_Transformer_F30_K7_1006_nonpermute_218_checkpoint.weights.h5'):
    model = get_model()
